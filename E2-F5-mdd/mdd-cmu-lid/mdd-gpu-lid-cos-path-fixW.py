...
##return a list of phoneme masks for each phoneme, before and after scaled by mask_ratio
def mdd_mask_fix( pid_seq, mask_ratio_min, avg_len, ratio_avg, device, neighbor_frame_num):  
    length = pid_seq[-1][-1]
    mask_list = []
    mask_list_orig = []
    for index in range(len(pid_seq)):
        ...
        r = math.ceil(r_orig + extend) if r_orig + extend <= length else length
        mask[l:r] = False ## False == masked!
        #using neighbor_frame_num for fixed jacobian size
        mid = int((l_orig + r_orig)/2)
        l_orig = mid - neighbor_frame_num
        assert l_orig >= 0 
        r_orig = mid + neighbor_frame_num
        assert r_orig <= length
        mask_orig[l_orig:r_orig+1] = False
        mask_list.append(mask)
        mask_list_orig.append(mask_orig)
    return mask_list, mask_list_orig
...
### non-batch version
def get_avg_posterior(model, text_in, cond, pid_seq, cfg_strength_gop=0, diff_symbol=None, masking_ratio_min=1, ratio_avg=1, sway_sampling_coef=-1, steps=32, n_samples=5, remove_first_t_back=False, use_null_diff=False, neighbor_frame_num=1):
    ...
    phoneme_mask_list, phoneme_mask_list_orig = mdd_mask_fix(pid_seq, masking_ratio_min, avg_len, ratio_avg, device, neighbor_frame_num)
    num_phonemes = len(pid_seq)

    if cfg_strength_gop != 0:
        ##dynamic batch_size
        amount = num_phonemes * duration_mel
        max_batch_new = int(12*(25000/amount))
    else:
        amount = num_phonemes * duration_mel
        max_batch_new = int(12*(25000/amount))
    ...
    for i in range(iter_num):
        ...
        gop_temp, gop_null_temp, trace_res_t, trace_res_null_t, lid_res_t, lid_res_null_t, plains_cos_t, plain_cos_null_t = model.compute_prob_hut_lid_cos_path( **input_kwargs)
        ...
        plain_cos_res += plains_cos_t
        plain_cos_res_null += plain_cos_null_t
    ...
    return gop, gop_null, trace_res, trace_res_null, lid_res, lid_res_null, plain_cos_res, plain_cos_res_null
...
def batch_process(batch, device, out_path=None):
    ...
    #use_null_diff =True
    use_null_diff =False
    neighbor_frame_num = 2
    
    if tokenizer == "pinyin" and diff_symbol is not None:
        diff_symbol = convert_char_to_pinyin([diff_symbol])
    
    print(f"Using cfg={cfg_strength_gop}, ratio_avg={ratio_avg}, steps={steps}, sway={sway_sampling_coef}, diff={diff_symbol}, remove_first_t_back={remove_first_t_back}")
    ...
    with torch.no_grad(), open(out_path+"_"+proc_id+".gop", "a") as f:
        for i,uid in enumerate(batch["id"]):
            ...
            gop_list, gop_null_list, trace_list, trace_list_null, lid_res, lid_res_null, plain_cos_res, plain_cos_res_null  = get_avg_posterior(model, tokens, mel, pid_seq, cfg_strength_gop=cfg_strength_gop, diff_symbol=diff_symbol, masking_ratio_min=masking_ratio_min, ratio_avg=ratio_avg, sway_sampling_coef=sway_sampling_coef, steps=steps, n_samples=n_samples, remove_first_t_back=remove_first_t_back, use_null_diff=use_null_diff, neighbor_frame_num=neighbor_frame_num)       
            assert len(pid_seq) == len(gop_list) and len(gop_list) == len(lid_res)
            ### write files      
            f.write(uid+'\n')
            for i, (gop, gop_null, trace, trace_null, lid, lid_null, plain_cos, plain_cos_null) \
                in enumerate(zip(gop_list, gop_null_list, trace_list, trace_list_null, lid_res, lid_res_null, plain_cos_res, plain_cos_res_null)):
                f.write("%d %s %s %s %s %s %s %s %s %s\n"%(i, pid_seq[i][0], gop, gop_null, ",".join([str(num) for num in trace]), ",".join([str(num) for num in trace_null]), ",".join([str(num) for num in lid]), ",".join([str(num) for num in lid_null]), ",".join([str(num) for num in plain_cos]), ",".join([str(num) for num in plain_cos_null])))
            f.write("\n")
