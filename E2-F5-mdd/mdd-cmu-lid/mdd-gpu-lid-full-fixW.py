...
def get_avg_posterior(model, text_in, cond, pid_seq, cfg_strength_gop=0, diff_symbol=None, masking_ratio_min=1, ratio_avg=1, sway_sampling_coef=-1, steps=32, n_samples=5, remove_first_t_back=False, use_null_diff=False, neighbor_frame_num=1):
    ...
    if cfg_strength_gop != 0:
        ...
        max_batch_new = int(6*(25000/amount))
    else:
        amount = num_phonemes * duration_mel
        #max_batch_new = int(num_phonemes*(25000/amount))
        max_batch_new = int(6*(25000/amount))
...
def batch_process(batch, device, out_path=None):
    ...
    with torch.no_grad(), open(out_path+"_"+proc_id+".gop", "a") as f:
        for i,uid in enumerate(batch["id"]):
            ...
            gop_list, gop_null_list, lid_res, lid_res_null, lid_count, lid_count_null = get_avg_posterior(model, tokens, mel, pid_seq, cfg_strength_gop=cfg_strength_gop, diff_symbol=diff_symbol, masking_ratio_min=masking_ratio_min, ratio_avg=ratio_avg, sway_sampling_coef=sway_sampling_coef, steps=steps, n_samples=n_samples, remove_first_t_back=remove_first_t_back, use_null_diff=use_null_diff, neighbor_frame_num=neighbor_frame_num)       
