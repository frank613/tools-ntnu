...
def batch_process(batch, device, out_path=None):
    ...
    with torch.no_grad(), open(out_path+"_"+proc_id+".gop", "a") as f:
        for i,uid in enumerate(batch["id"]):
            ...
            gop_list, gop_null_list, trace_list, trace_list_null, lid_res, lid_res_null = get_avg_posterior(model, tokens, mel, pid_seq, cfg_strength_gop=cfg_strength_gop, diff_symbol=diff_symbol, masking_ratio_min=masking_ratio_min, ratio_avg=ratio_avg, sway_sampling_coef=sway_sampling_coef, steps=steps, n_samples=n_samples, remove_first_t_back=remove_first_t_back, use_null_diff=use_null_diff, neighbor_frame_num=neighbor_frame_num)       
            assert len(pid_seq) == len(gop_list) and len(gop_list) == len(lid_res) and len(lid_res) == len(trace_list)
            ### write files      
            f.write(uid+'\n')
