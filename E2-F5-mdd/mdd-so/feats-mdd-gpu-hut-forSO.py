...
def get_avg_posterior(model, text_in, cond, pid_seq, cfg_strength_gop=0, diff_symbol=None, masking_ratio=1, sway_sampling_coef=-1, steps=32, n_samples=5, remove_first_t_back=False, use_null_diff=False):
    ...
    for i in range(iter_num):
        ...
        ## Hut approximation, return a list of (B x T), gop and gop_null
        gop_temp, gop_diff_temp = model.compute_prob_hut_mask_feats( **input_kwargs)
        gop = gop + gop_temp
        gop_diff = gop_diff + gop_diff_temp
...
def batch_process(batch, device, out_path=None):
    ...
    with torch.no_grad(), open(out_path+"_"+proc_id+".gop", "a") as f:
        for i,uid in enumerate(batch["id"]):
            ...
            gop_list, gop_null_list = get_avg_posterior(model, tokens, mel, pid_seq, cfg_strength_gop=cfg_strength_gop, diff_symbol=diff_symbol, masking_ratio=masking_ratio, sway_sampling_coef=sway_sampling_coef, steps=steps, n_samples=n_samples, remove_first_t_back=remove_first_t_back, use_null_diff=use_null_diff)  
            assert len(pid_seq) == len(gop_list) and len(gop_list) == len(gop_null_list)
            ### write files      
            f.write(uid+'\n')
            for i, (gop, gop_diff) in enumerate(zip(gop_list ,gop_null_list)):
                gop = [ str(score) for score in gop ]
                gop_diff = [ str(score) for score in gop_diff]
                f.write("%d %s %s %s\n"%(i, pid_seq[i][0], ",".join(gop), ",".join(gop_diff)))
            f.write("\n")
