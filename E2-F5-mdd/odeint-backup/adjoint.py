...
class OdeintAdjointMethod(torch.autograd.Function):
    ...
    def forward(ctx, shapes, func, y0, t, rtol, atol, method, options, event_fn, adjoint_rtol, adjoint_atol, adjoint_method,
        ...
        with torch.no_grad():
            ...
            if event_fn is None:
                y = ans ###Xinwei: the the final state at y(t=1) is the only thing needs to be passed to backward, so O(1) of memory is achieved via adjoint. 
                        ###Here, I guess all the intermidiate states(trajectroy) are tracked. So no need of doing inverse ODE for states again in Backward
    ...
    def backward(ctx, *grad_y):
        with torch.no_grad():
            ...
            if event_mode:
                ...
                grad_y = grad_y[0] #Xinwei: the "vector" of the vector-jacobian-product, usually from upper layers or all "1" for the final layer

            adjoint_params = tuple(adjoint_params) ##Xinwei: model parameters
            ...
            ##Xinwei: only needs last state y[-1], but why? already have all the ys from forward. Look below, no need to compute again y[i-1]
            ##Xinwei: why does grad_y have length t?
            ...
            def augmented_dynamics(t, y_aug):
                ...
                with torch.enable_grad():
                    ...
                    vjp_t, vjp_y, *vjp_params = torch.autograd.grad(
                        func_eval, (t, y) + adjoint_params, -adj_y,  ###Xinwei: here the -adj_y reuses the "input_grad" of back-propagation
                                                                     ###also in the appendix of the paper, it mentions the analog of BP and adjoint method.
