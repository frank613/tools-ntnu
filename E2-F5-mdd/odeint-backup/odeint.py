...
SOLVERS = {
    ...
    'euler': Euler,
    'euler_mdd': Euler_MDD, ##XINWEI: EULER for MDD
    'midpoint': Midpoint,
    'heun2': Heun2,
    'heun3': Heun3,
    'rk4': RK4,
...
##XINWEI: MDD
def odeint_jacobian_aabb(func, y0, t, *, rtol=1e-7, atol=1e-9, method=None, options=None, event_fn=None):
    """Integrate a system of ordinary differential equations.
    ...
    solver = Euler_MDD_aabb(func=func, y0=y0, rtol=rtol, atol=atol, **options)

    if event_fn is None:
        solution, jacob_trace = solver.integrate(t)
    else:
...
##XINWEI: MDD
def odeint_jacobian_aabb_fix(func, y0, t, t_interval, *, rtol=1e-7, atol=1e-9, method=None, options=None, event_fn=None):
    """Integrate a system of ordinary differential equations.
    ...
    solver = Euler_MDD_aabb_fix(func=func, y0=y0, rtol=rtol, atol=atol, **options)

    if event_fn is None:
        solution, jacob_trace = solver.integrate(t, t_interval)
    else:
        sys.exit("not supported in this MDD version")
...
##XINWEI: MDD 
def odeint_dist(func, y0, t, *, rtol=1e-7, atol=1e-9, method=None, options=None, event_fn=None):
    """Integrate a system of ordinary differential equations.
    ...
    solver = Euler_MDD_Dist(func=func, y0=y0, rtol=rtol, atol=atol, **options)

    if event_fn is None:
        solution, dist_list = solver.integrate(t)
    else:
...
##XINWEI: Hut approximation return gop directly, LID fixing
def odeint_jacobian_hut_lid(func, y0, t, cond_mask, n_samples, *, rtol=1e-7, atol=1e-9, method=None, options=None, event_fn=None):
    """Integrate a system of ordinary differential equations.
    ...
    solver = Euler_MDD_Hut_Lid(func=func, y0=y0, rtol=rtol, atol=atol, **options)

    if event_fn is None:
        solution, jacob_trace, lid_res_mean, lid_res_norm, lid_count, lid_log_mean = solver.integrate(t, cond_mask, n_samples)
    else:
        sys.exit("not supported in this MDD version")
    ...
    return solution, jacob_trace, lid_res_mean, lid_res_norm, lid_count, lid_log_mean  

##XINWEI: Hut approximation return gop directly, LID fixing, only the first 8 steps
def odeint_jacobian_hut_lid_832(func, y0, t, cond_mask, n_samples, *, rtol=1e-7, atol=1e-9, method=None, options=None, event_fn=None):
    """Integrate a system of ordinary differential equations.
    ...
    solver = Euler_MDD_Hut_Lid_832(func=func, y0=y0, rtol=rtol, atol=atol, **options)
...
def odeint_jacobian_hut_lid_energy(func, y0, t, cond_mask, n_samples, *, rtol=1e-7, atol=1e-9, method=None, options=None, event_fn=None):
    """Integrate a system of ordinary differential equations.
    ...
    solver = Euler_MDD_Hut_Lid_Energy(func=func, y0=y0, rtol=rtol, atol=atol, **options)
...
def odeint_jacobian_hut_lid_full(func, y0, t, cond_mask, n_samples, *, rtol=1e-7, atol=1e-9, method=None, options=None, event_fn=None):
    ...
    solver = Euler_MDD_Hut_Lid_Full(func=func, y0=y0, rtol=rtol, atol=atol, **options)

    if event_fn is None:
        solution, jacob_trace, lid_res, lid_count = solver.integrate(t, cond_mask, n_samples)
    else:
        sys.exit("not supported in this MDD version")
    ...
    return solution, jacob_trace, lid_res, lid_count  
 
##XINWEI: Hut approximation return gop directly
def odeint_jacobian_hut_fix(func, y0, t, cond_mask, n_samples, *, rtol=1e-7, atol=1e-9, method=None, options=None, event_fn=None):
    """Integrate a system of ordinary differential equations.
    ...
    solver = Euler_MDD_Hut_fix(func=func, y0=y0, rtol=rtol, atol=atol, **options)
...
##XINWEI: Hut approximation return gop directly, speechocean762 version
def odeint_jacobian_hut_so(func, y0, t, cond_mask, n_samples, *, rtol=1e-7, atol=1e-9, method=None, options=None, event_fn=None):
    """Integrate a system of ordinary differential equations.
    ...
    solver = Euler_MDD_Hut_SO(func=func, y0=y0, rtol=rtol, atol=atol, **options)
...
##XINWEI: Hut approximation return gop directly
def odeint_jacobian_hut_full(func, y0, t, n_samples, *, rtol=1e-7, atol=1e-9, method=None, options=None, event_fn=None):
    """Integrate a system of ordinary differential equations.
    ...
    solver = Euler_MDD_Hut_full(func=func, y0=y0, rtol=rtol, atol=atol, **options)

    if event_fn is None:
        solution, jacob_trace = solver.integrate(t, n_samples)
    else:
        sys.exit("not supported in this MDD version")
...
##XINWEI: return jacobian and the trajectory
def odeint_jacobian(func, y0, t, cond_mask, t_inerval, cfg, *, rtol=1e-7, atol=1e-9, method=None, options=None, event_fn=None):
    """Integrate a system of ordinary differential equations.
    ...
    solver = Euler_MDD(func=func, y0=y0, rtol=rtol, atol=atol, **options)

    if event_fn is None:
        solution, jacob_trace = solver.integrate(t, cond_mask, t_inerval, cfg)
    else:
        sys.exit("not supported in this MDD version")
...
##XINWEI: return jacobian and the trajectory
def odeint_jacobian_test(func, y0, t, cond_mask, t_inerval, cfg, *, rtol=1e-7, atol=1e-9, method=None, options=None, event_fn=None):
    """Integrate a system of ordinary differential equations.
    ...
    solver = Euler_MDD_test(func=func, y0=y0, rtol=rtol, atol=atol, **options)
...
##XINWEI: return jacobian and the trajectory
def odeint_jacobian_wrong(func, y0, t, cond_mask, cfg, *, rtol=1e-7, atol=1e-9, method=None, options=None, event_fn=None):
    """Integrate a system of ordinary differential equations.
    ...
    shapes, func, y0, t, rtol, atol, method, options, event_fn, t_is_reversed = _check_inputs(func, y0, t, rtol, atol, method, options, event_fn, SOLVERS)
    assert method == "euler_mdd"
    solver = Euler_MDD_Wrong(func=func, y0=y0, rtol=rtol, atol=atol, **options)

    if event_fn is None:
        solution, jacob_trace = solver.integrate(t, cond_mask, cfg)
    else:
        sys.exit("not supported in this MDD version")
