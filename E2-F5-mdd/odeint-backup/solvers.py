    ...
    def integrate(self, t):
        ...
        with torch.enable_grad():
            ...
            for i, (t0, t1) in enumerate(iterator):
                ...
                dt = t1 - t0
                #self.func.callback_step(t0, y0, dt)
                #if self.func.callback_step is not None:
                #    sys.exit("call back is not supported for MDD")
                dy, f0 = self._step_func(self.func, t0, dt, t1, y0)
                ## negate for MDD(forward)
                f0 = -f0
                sum = torch.zeros(y0.shape[0], y0.shape[1], device=self.y0.device)
                for j in range(y0.shape[-1]):
                    grad_out = torch.zeros_like(y0)
                    grad_out[:,:,j] = 1 
                    sum += torch.autograd.grad(f0, y0, grad_out, retain_graph=True)[0][:,:,j] ## B x T
                jacob_trace[i] = sum
