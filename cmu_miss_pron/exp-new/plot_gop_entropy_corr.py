...
def print_corr(corr_df):
    ...
    print(corr_df['AUC_t'].corr(corr_df['AUC_stud']))
    print("overall correlation between L and AUC Student:")
    print(corr_df['L'].corr(corr_df['AUC_stud']))
    methods = corr_df['method'].unique()
    for mtd in methods:
        print("Method: {} correlation between AUC Teacher and AUC Student:".format(mtd))
        print(corr_df.loc[corr_df["method"]==mtd, "AUC_t"].corr(corr_df.loc[corr_df["method"]==mtd, "AUC_stud"]))
        print("Method: {} correlation between L and AUC Student:".format(mtd))
        print(corr_df.loc[corr_df["method"]==mtd, "L"].corr(corr_df.loc[corr_df["method"]==mtd, "AUC_stud"]))
    ...
    for pair in pairs:
        ...
        #relative diff
        #diff_L = diff_L /corr_df.loc[corr_df["method"]==pair[1], "L"].to_numpy()
        print(np.corrcoef(diff_L, diff_auc_stud))
