...
def labelError(GOP_file, error_list, tran_file, index):
    gop_df = readGOPToDF(GOP_file, index)
...
def readGOPToDF(ark_file, index):
    ...
    for line in in_file:
        ...
        cur_score_list = fields[2].split(",")
        if len(cur_score_list) != 8:
            sys.exit("gop must have 8 numbers")
...
if __name__ == "__main__":
    ...
    json_dict, avg_auc = labelError(sys.argv[1], utt_list, sys.argv[3], index)
    os.makedirs(os.path.dirname(sys.argv[4]), exist_ok=True)
    with open(sys.argv[4], "w") as f:
        json.dump(json_dict, f)

    print(f'The avg AUC using code layer {index}: {avg_auc}')
