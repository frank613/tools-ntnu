...
def readGOP_feats_kaldi(gop_file, p_table):
    df_temp = {}
    count = 0
    cur_uttid = ""
    seq_score = []
    for uttid_ph, feat in kaldi_io.read_vec_flt_scp(gop_file):
        uttid = uttid_ph.split('.')[0]
        if uttid != cur_uttid:
            if uttid not in p_table.keys():
                    sys.exit(f'no human score for {uttid}')
            if len(seq_score) != 0:
                assert count == len(p_table[cur_uttid][0]) 
                df_temp.update({cur_uttid:[(p,g,l) for (p,g),l in zip(seq_score, label_phoneme)]})
                seq_score = []
                count = 0
            label_phoneme = p_table[uttid][1]
            phs = p_table[uttid][0]
            cur_uttid = uttid
        seq_score.append((phs[count],feat.tolist()))
        count += 1
    ##rest of the loop
    if len(seq_score) != 0: 
        assert count == len(p_table[cur_uttid][0])
...
def readGOP_feats(gop_file, p_table, diff=False):
    ...
    for line in in_file:
        ...
        if diff == "all":
            cur_score = [float(x) for x in fields[2].split(',')] + [float(x) for x in fields[3].split(',')]
        else:
            if diff == True:
                field_sec = 3
            else:
...
def merge_features(data_list_one, data_list_two, number_of_features=8, dummy=False):
    df = {}
    for uttid, features in data_list_one.items():
        ...
        for i,(p,g,l) in enumerate(features):
            ...
            if dummy:
                new_features.append((p,g+[0]*number_of_features,l))
            else:
