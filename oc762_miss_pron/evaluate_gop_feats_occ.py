...
def merge_features(data_list_one, data_list_two, normalize=False, occ_number=None):
    df = {}
    for uttid, features in data_list_one.items():
        ...
        for i,(p,g,l) in enumerate(features):
            ...
            norm_fact = max(features_two[i][1][0], 1)
            if occ_number is not None:
                add_list = features_two[i][1][occ_number:occ_number+1]
            else:
            ...
            if not normalize:
                ...
                #41 very bad
                #new_features.append((p,g[1:]+add_list,l))
            else:
...
if __name__ == "__main__":
    if len(sys.argv) != 6:
        sys.exit("this script takes 5 arguments <GOP-feats file> <GOP-occ file> <metadata.csv> <train-utt2dur-kaldiformat> <test-utt2dur-kaldiformat>. It labels the phonemes in the GOP file based on the annotation file, learns a SVR model, predict the test set, outputs a summary ")
    merge_feats =True
    #readfiles
    p_dict = read_anno(sys.argv[3])
    occ_list = readGOP_OCC(sys.argv[2], p_dict)
    ...
    if merge_feats:
        ...
        data_list = merge_features(feats_list, occ_list, normalize=False, occ_number=None)
    else:
