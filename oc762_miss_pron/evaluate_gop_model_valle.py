...
def readGOP_kaldi(gop_file, p_table):
    df_temp = []
    for uttid, gops in kaldi_io.read_post_scp(gop_file):
        if uttid not in p_table.keys():
                sys.exit(f'no human score for {uttid}')
        assert len(p_table[uttid][0]) == len(gops)
        label_phoneme = p_table[uttid][1]
        phs = p_table[uttid][0]
        seq_score = []
        for i, [(_, gop)] in enumerate(gops):
            seq_score.append((phs[i],gop))
        df_temp.append((uttid, [(p,g,l) for (p,g),l in zip(seq_score, label_phoneme)])) 
    return df_temp

def readGOP(gop_file, p_table, level=0, merged=True):
