...
def plot(target_dict, ref_dict_list, target_dname, ref_dname, out_file):
    ...
    target_pos = np.arange(len(phoneme_list)+1)*1.5
    ref_pos_list = [target_pos+(i+1)*barWidth for i in range(len(ref_dict_list))]
     
    # Make the plot
    fig, ax = plt.subplots(figsize=(20, 5))
    auc_target = [ target_dict["phonemes"][p][0] for p in phoneme_list] + [target_dict["summary"]["average-AUC"]]
    ...
    for i, r_dict in enumerate(ref_dict_list): 
        auc_ref = [ r_dict["phonemes"][p][2] for p in phoneme_list] + [r_dict["summary"]["average-AUC"]]
    ...
    plt.xticks(target_pos + barWidth,list(phoneme_list)+["ALL"], rotation='vertical')
