...
if __name__ == "__main__":
    ...
    data=pd.DataFrame(columns=["dataset","phoneme","auc"])

    for phoneme in target_dict["phonemes"].keys():

        data.append("cmu-real", phoneme, target_dict["phonemes"][phoneme][0]


    for phoneme in ref_dict_list[0]["phonemes"].keys():
        data.apppend("cmu")



    sns.barplot(data=data, x="phoneme",  y="auc", hue="dataset")
