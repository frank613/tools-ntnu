...
def compute_post_sum(decodable, trMod, transSeq, start_pos, length):
    log_sum = 0
    for i in range(start_pos, start_pos + length): 
        logpost_vec = get_vector_for_frame(decodable, i)
        log_sum += logpost_vec._getitem(trMod.transition_id_to_pdf(transSeq[i-start_pos]))
    return log_sum

def compute_post_replaced(decodable, trMod, transSeq, tree, context, start_pos, length):
    log_sum = 0
    # print(tree.context_width())
    for i in range(start_pos, start_pos + length):
        logpost_vec = get_vector_for_frame(decodable, i)
        log_sum += logpost_vec._getitem(tree.compute(context, trMod.transition_id_to_pdf_class(transSeq[i-start_pos])))
    return log_sum
        ...
        for frame_num, (fkey, feats) in enumerate(f):
            ...
            for order, (p_id,start_idx) in enumerate(pid_seq):
                ...
                if p_sym_raw in p_set:
                    real_post = compute_post_sum(decodable, trans_m, segmented[order], start_idx, length_seg)
                    gops_real[p_sym_raw].append(real_post/length_seg)     
                    for p in p_set - set([p_sym_raw]):
                        toP = p_map.switch_symbol(p_sym, p)
                        toPid = p_map.get_phone_id(toP)
                        context = [toPid]
                        post = compute_post_replaced(decodable, trans_m, segmented[order], tree, context, start_idx, length_seg)
                        post_avg = post/length_seg
                        gops_map[p_sym_raw][p].append(post_avg)         
