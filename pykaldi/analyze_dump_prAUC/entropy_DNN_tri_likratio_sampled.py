...
def write(results, gops_real, outFile):
    ...
    #p:(closest_phoneme, mean_diff, auc_value, pr_auc, entropy, count_of_real, count_of_error)
    out_form = { \
                'phonemes':{},  
                'summary': {"average-mean-diff": None, "average-AUC": None, "average-pr-AUC": None, "total_real": None, "total_error":None}} 
    #count of phonemes}
    total_real = 0
    total_error = 0
    total_auc = 0
    ...
    for (k,v) in gops_real.items():
        ...
        for p in set(gops_real.keys()) - set([k]):
            ...
            if len(real_arr) > len(sub_arr):
                real_arr_sampled = np.random.choice(real_arr, len(sub_arr))
                sub_arr_sampled = sub_arr
            if len(sub_arr) > len(real_arr):
                sub_arr_sampled = np.random.choice(sub_arr, len(real_arr))
                real_arr_sampled = real_arr

            real_label = np.stack((real_arr_sampled, np.full(len(real_arr_sampled), 0)), 1)
            sub_label = np.stack((sub_arr_sampled, np.full(len(sub_arr_sampled), 1)), 1)
            auc_value, pr_auc = auc_cal(np.concatenate((real_label, sub_label)))
            scores.append((p, sub_arr.mean(), len(sub_arr), pr_auc, auc_value))
            total_error += len(sub_arr)
        
        confused_p, p_mean, num_error, pr_auc, auc = sorted(scores, key = lambda x: x[3])[0]
        mean_diff = round(real_arr.mean() - p_mean, 3)
        out_form["phonemes"][k] = (confused_p, mean_diff, auc, pr_auc, ent, len(v), num_error) 
        total_auc += auc
        total_pr_auc += pr_auc
    ...
    out_form["summary"]["average-pr-AUC"]=total_pr_auc/len(gops_real.items())
