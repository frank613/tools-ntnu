...
def plot(results, fromP):
    ...
    for ax, (k,v) in zip(axs, results.items()):
        sub_arr = np.array(v)
        ax.hist(sub_arr, color='g', density=True, range=[-100, 10], bins=100, histtype='stepfilled', alpha=0.5, label='real')
        ax.axvline(sub_arr.mean(), color='g', linestyle='dashed', linewidth=1, label='substituted-mean')

        real_arr = np.array(results[fromP])
        ax.hist(real_arr, color='r', density=True, range=[-100, 10], bins=100, histtype='stepfilled', alpha=0.5, label='real')
        ax.axvline(real_arr.mean(), color='r', linestyle='dashed', linewidth=1, label='canonial-mean')  
        ax.legend(loc ="upper left")
        ax.set_title('{0}<-{1}, Diff_mean={2}'.format(k, fromP, round(real_arr.mean() - sub_arr.mean(), 3)))
    outFile = "./output-gop/{0}/all.png".format(fromP)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 7:
        ...
        it replaces the original alignment for the target phoneme with other phonemes and compute the GOP for those replaced segments accrodingly.\n \
        Then, it plots the GOP of each segment of the target phoneme versus its replaced version")
    ...
    gops_map.update({targetP:[]}) #add the original one
