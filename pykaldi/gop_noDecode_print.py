...
def compute_likeli_ali(acMod, trMod, transSeq, featureSeq):
    ...
    for t_id,features in zip(transSeq, featureSeq):
        score = acMod.log_likelihood(trMod.transition_id_to_pdf(t_id), features)
        results += score
...
def get_likeli_for_denom_perframe(aligner, featureSeq, phones, text_list):  #return the best phoneme for the given feature segments and the total sum of likelihood
    ...
    for text in text_list:
        ...
        if  -results['weight'].value2 > max_like:
            ...
            #print("processed: {0}, score: {1}".format(text, results['likelihood']))
    am = aligner.acoustic_model
    tm = aligner.transition_model
    assert len(featureSeq) == len(best_ali)
    like_list = [ am.log_likelihood(tm.transition_id_to_pdf(tid), feats) for tid,feats in zip(best_ali, featureSeq)]
    #phone_alignment = aligner.to_phone_alignment(best_ali, phones)
    #best_phoneme = aligner.transition_model.transition_id_to_phone(best_ali[0])
    #print(best_phoneme)
    #pdb.set_trace()
    #negte it because the lattice has positve scores
    return (like_list, max_like)
...
if __name__ == "__main__":
    ...
    print(sys.argv)
    if len(sys.argv) != 8:
        sys.exit("this script takes 7 arguments <cano-alignment file> <model-dir> <phones.txt> <data-dir> <lexicon-file-for-denom> <words-symbol-for-denom> <uttid>.\n \
    ...
    with SequentialMatrixReader(feats_rspecifier) as f:
        for fkey, feats in f:
            ...
            for order, (p_id,start_idx) in enumerate(pid_seq):
                ...
                denom_list, denom_sum = get_likeli_for_denom_perframe(aligner, feats[start_idx: start_idx + length_seg].clone(), phone_symbols, fake_text_list)
                #print(p_map.get_raw_symbol(pid_best))
                p_sym_raw = p_map.get_raw_symbol(p_id)
                p_sym = p_map.get_symbol(p_id)
                num_list, num_lik = compute_likeli_ali(acoustic_m, trans_m, segmented[order], feats[start_idx: start_idx + length_seg])
                assert len(num_list) == len(denom_list)
                #pdb.set_trace()
                gop_score = (num_lik - denom_sum )/length_seg
                print("Phoneme {0}, GOP {1}".format(p_sym, str(gop_score)))
                for i, (num, denom) in enumerate(zip(num_list, denom_list)):
                    print("{0} {1} {2}".format(i, num, denom))
