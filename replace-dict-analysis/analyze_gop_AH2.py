...
def plot(df_labels, df_labels2, from_phoneme, phoneme):
    ...
    if len(substituted) != 0:
        ...
        ax[0].set_title('Poistion-dependent model, phone-align: {0}<-{1}, AUC={2}, Diff_mean={3}'.format(phoneme, from_phoneme, round(auc_cal(auc_arr),3), round(real_mean - sub_mean, 3)))
    ...
    ax[1].set_title('Position-independent model, phone-align: {0}<-{1}, AUC={2}, Diff_mean={3}'.format(phoneme, from_phoneme, round(auc_cal(auc_arr2),3), round(real_mean2 - sub_mean2, 3)))

    html_str = mpld3.fig_to_html(fig)
    Html_file= open("./output_AH2/{0}/{1}.html".format(phoneme, from_phoneme),"w")
    Html_file.write(html_str)
    Html_file.close()     

    plt.savefig("./output_AH2/{0}/{1}.png".format(phoneme, from_phoneme))
