...
def plot(df_labels, df_labels2, from_phoneme, phoneme):
    ...
    if len(substituted) != 0:
        ...
        ax[0].set_title('Re-align: {0}<-{1}, AUC={2}, Diff_mean={3}'.format(phoneme, from_phoneme, round(auc_cal(auc_arr),3), round(real_mean - sub_mean, 3)))
    ...
    ax[1].set_title('Fix-align: {0}<-{1}, AUC={2}, Diff_mean={3}'.format(phoneme, from_phoneme, round(auc_cal(auc_arr2),3), round(real_mean2 - sub_mean2, 3)))

    html_str = mpld3.fig_to_html(fig)
    Html_file= open("./output2/{0}/{1}.html".format(phoneme, from_phoneme),"w")
    Html_file.write(html_str)
    Html_file.close()     

    plt.savefig("./output2/{0}/{1}.png".format(phoneme, from_phoneme))

if __name__ == "__main__":
    print(sys.argv)
    if len(sys.argv) != 6:
        sys.exit("this script takes 5 arguments <GOP file1> <GOP file2> <cano GOP file> <phoneme-orig> <phoneme-to> . \
        It labels the substituted phonemes and plot the points with the labels")

    df_labels = labelError(sys.argv[1], sys.argv[3], sys.argv[4])
    print("label error done for the first gop")
    df_labels2 = labelError2(sys.argv[2], sys.argv[4])
    print("label error done for the second gop")
    plot(df_labels, df_labels2, sys.argv[4], sys.argv[5])
    #showGOP(df_gop)
