...
def read_gop(gopFile):
    ...
    #df = pd.DataFrame(columns=('uttid', 'score', 'symbol'))
    isNewUtt = True
    gopList = []
    for line in in_file:
        line = line.strip()
    ...
    return pd.DataFrame(gopList, columns=('symbol', 'score', 'uttid'))
...
def trim_left(in_df, symbol, prob):
        score_serie= in_df[in_df.symbol == symbol].score
        sorted_s = score_serie.sort_values()
        #trimmed = score_serie[(score_serie < score_serie.quantile(1-prob)) & ( score_serie > score_serie.quantile(prob))]
        length = math.floor(len(sorted_s)*prob)
        mask = [ False if i < length else True for i in range(len(sorted_s))]
        trimmed = sorted_s[mask]
        return trimmed

def trim_right(in_df, symbol, prob):
        score_serie= in_df[in_df.symbol == symbol].score
        sorted_s = score_serie.sort_values()
        #trimmed = score_serie[(score_serie < score_serie.quantile(1-prob)) & ( score_serie > score_serie.quantile(prob))]
        length = math.floor(len(sorted_s)*prob)
        mask = [ False if i > len(sorted_s)-1-length else True for i in range(len(sorted_s))]
        trimmed = sorted_s[mask]
        return trimmed
...
def qq_plot(gopFile, symbol, outFile):
    ...
    #qqplot(-1*(gop_df.loc[gop_df["symbol"]== symbol, 'score']), dist=expon ,line='s')
    trimmed = trim_right(gop_df, symbol, 0.4)
    qqplot(-1*trimmed, dist=expon ,line='s')
