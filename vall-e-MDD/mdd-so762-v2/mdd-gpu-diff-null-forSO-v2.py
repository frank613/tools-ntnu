...
def compute_gop(out_probs, out_probs_diff, phoneme_mask_list):
    gop_list = [ prob[mask].log().mean(dim=0) if (prob[mask] == 0).sum() == 0 else sys.exit("not valid output") for prob, mask in zip(out_probs, phoneme_mask_list)]
    gop_list_diff = [ prob[mask].log().mean(dim=0) if (prob[mask] == 0).sum() == 0 else sys.exit("not valid output") for prob, mask in zip(out_probs_diff, phoneme_mask_list)]
    gop_list_diff = [ gop_diff - gop for gop, gop_diff in zip(gop_list, gop_list_diff)]
    pdb.set_trace() #check dims
    return gop_list.tolist(), gop_list_diff.tolist()
