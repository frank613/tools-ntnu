...
def load_dataset_local_from_dict(csv_path, cache_additional, trans_map, uttid_list, lang_code):
        ...
        def map_to_array(batch):   
            ...
            for i, uid in enumerate(batch["id"]):
                if uid not in uttid_list:
                    ...
                    prompt, resp = get_emb(audio_in_path, trim_length=3, noise=True)
...
def batch_process(batch, p_tokenizer, device, out_path):
    #pdb.set_trace()
...
if __name__ == "__main__":
    ...
    print(sys.argv)
    if len(sys.argv) != 7:
        sys.exit("this script takes 6 arguments <model-ckpt-sft> <in-audio-csv-folder> <in-raw-text-file> <CTM-alignment-file> <gop-tokenizer-path> <out-gop-path> \n \
    ...
    ds= load_dataset_local_from_dict(csv_path, "cmu-kids-noise", trans_map, uttid_list, lang_code="en-us")
    ...
    ds.map(batch_process, fn_kwargs={"p_tokenizer":p_tokenizer, "device":device, "out_path":out_path}, batched=True, batch_size=50, num_proc=20)
