...
def get_tts_logtis_and_plot(model, text_in, prop_in, lang, is_ar, device, reps_in, predict_level_0=True, pid_seq=None, mask_index=None, target_phoneme=None, mask_ratio=1, out_path=None): 
    ...
    for i in range(1):
        ...
        plot_activations(logit_list[0].transpose(0,1).transpose(1,2), resps_out, reps_in, target_phoneme, left=l, right=r, out_path=out_path+f"-plot-{mask_index}_{target_phoneme}-{i}.png")
...
if __name__ == "__main__":
    ...
    set_seed()
    with torch.no_grad():
        get_tts_logtis_and_plot(models[0], phns, prompt, lang, False, device, resps, predict_level_0=True, pid_seq=pid_seq, mask_index=mask_index, target_phoneme=target_phoneme, mask_ratio=mask_ratio, out_path=sys.argv[6])
