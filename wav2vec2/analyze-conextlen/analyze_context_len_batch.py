...
def get_alpha_bar(alphas, t, blank, pos, next_label, params):
    #if pos == 9 and t == 75:
    #   pdb.set_trace()
    arbitrary_state = 2*pos + 1 
    alpha_mask = torch.ones(alphas.shape[2], dtype=torch.bool)
    ...
    if t != 0 and next_label is not None:
        extra = alphas[arbitrary_state, t-1, next_label] * params[blank, t]
        ret = alphas[:arbitrary_state,t,0].sum() + alphas[arbitrary_state+1:,t,0].sum() + alphas[arbitrary_state,t,alpha_mask].sum() - extra
    else:
        ret = alphas[:arbitrary_state,t,0].sum() + alphas[arbitrary_state+1:,t,0].sum() + alphas[arbitrary_state,t,alpha_mask].sum()
    return ret
...
def ctc_loss_denom(params, seq, pos, blank=0):
    ...
    if pos == 0:
        ...
        alpha_bar[0] = get_alpha_bar(alphas, 0, blank, pos, next_label_idx, params)
    ...
    for t in range(1,T):
        ...
        #if t == 14:
        #   pdb.set_trace()
        alpha_bar[t] = get_alpha_bar(alphas, t, blank, pos, next_label_idx, params)
        alphas[:,t,:] = alphas[:,t,:] / alpha_bar[t]  
