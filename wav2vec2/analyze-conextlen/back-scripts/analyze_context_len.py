...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        for row in ds:
            ...
            #run the test for context len
            ### from left 
            p_index = 0
            cur_p = pid_seq[p_index]
            if p_index + context_len > len(pid_seq) - 1:
                print("not enough right context")  
                break
            print(f"testing the left most phoneme: {processor.tokenizer._convert_id_to_token(cur_p)}")
            for i in range(0, context_len+1):
                labels = torch.Tensor(pid_seq[0:i+1]).type(torch.int32) ## start from no context
                frame_range_l = int(sum(token_range[(2*p_index + 1) -1])/2) 
                ...
                ll_denom = ctc_loss_denom(post_mat[frame_range_l:frame_range_r+1,:].transpose(0,1), labels, 0 , blank=0)
                #ll_self = ctc_loss(post_mat.transpose(0,1), labels, blank=0)
                #ll_denom = ctc_loss_denom(post_mat.transpose(0,1), labels, p_index , blank=0)
                gop = -ll_self + ll_denom
                print(f"GOP with context {i}: {gop}")
                
            ### from right
            p_index = len(pid_seq) - 1
            cur_p = pid_seq[p_index]
            if p_index - context_len < 0:
                print("not enough left context")  
                break
            print(f"testing the right most phoneme: {processor.tokenizer._convert_id_to_token(cur_p)}")
            for i in range(0, context_len+1):
                labels = torch.Tensor(pid_seq[p_index-i:]).type(torch.int32) ## start from no context
                frame_range_l = int(sum(token_range[(2*(p_index-i) + 1) -1])/2) + 1 ##left + 1, in case of skppied blank
                frame_range_r = int(sum(token_range[(2*(p_index) + 1) + 1])/2)
                ll_self = ctc_loss(post_mat[frame_range_l:frame_range_r+1,:].transpose(0,1), labels, blank=0)
                ...
                gop = -ll_self + ll_denom
                print(f"GOP with conext {i}: {gop}")
                
            ### from middle
            p_index = int(len(pid_seq)/2) - 1
            cur_p = pid_seq[p_index]
            if p_index - context_len/2 < 0 or p_index + context_len/2 > len(pid_seq) - 1:
                print("not enough left or right context")  
                break
            print(f"testing the phoneme in the middle: {processor.tokenizer._convert_id_to_token(cur_p)}")
            for i in range(0, int(context_len/2)+1):
                ...
                ll_denom = ctc_loss_denom(post_mat[frame_range_l:frame_range_r+1,:].transpose(0,1), labels, i, blank=0)
                gop = -ll_self + ll_denom
                print(f"GOP with context {2*i}: {gop}")
