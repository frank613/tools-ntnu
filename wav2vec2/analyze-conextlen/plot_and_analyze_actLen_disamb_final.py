...
def plot_it(out_file,rep,correct_gop, correct_gop_norm, wrong_gop, wrong_gop_norm, occ_vec):
   
    fig, ax = plt.subplots()
    x_ticks = np.arange(1,rep+1)
    #plt.rcParams['font.size'] = 50
    plt.xlabel('Activation length')
    plt.xticks(x_ticks)
    plt.yticks((-80,-70,-60,-50,-40,-30,-20,-10,0,10,20))
    plt.ylabel('GOP')
    plt.title('The GOP-SD-AF value at different activation length')
    plt.ylim(-80,20)


    ax.plot(x_ticks, occ_vec, 'o-',  label="occurance")
    ax.plot(x_ticks, correct_gop, 'om-',  label="correct")
    ax.plot(x_ticks, correct_gop_norm, 'o', linestyle='dashed', label="correct normalized")
    ax.plot(x_ticks, wrong_gop, 'o-',  label="subsituion")
    ax.plot(x_ticks, wrong_gop_norm, 'o', linestyle='dashed', label="substitution normalized")
    ...
    with torch.no_grad():
        ...
        for row in ds:
            ...
            for i in range(1, frame_rep+1):
                labels = torch.Tensor(pid_seq[p_index - context_range:p_index + context_range + 1]).type(torch.int32)
                ...
                ll_denom,occ = ctc_loss_denom(repeated, labels, context_range, blank=0)
                gop =(-ll_self + ll_denom).item()
                occ = occ.item()
                correct_gop.append(gop)
                correct_gop_norm.append(gop/occ)
                occ_vec.append(occ)
            ...
            pdb.set_trace()
            plot_it(sys.argv[5], frame_rep, correct_gop, correct_gop_norm, wrong_gop, wrong_gop_norm, occ_vec)
            break
