...
def plot_it(out_file, in_df):
    ...
    fig, axes = plt.subplots(len(token_list),3,figsize=(15*len(token_list), 150), sharex=True, sharey=True, layout="constrained")

    for i in range(len(token_list)):
        axes_id = i    
        all_df = in_df.loc[(in_df['token'] == token_list[i])]
        g = sns.lineplot(data=all_df, x="context-len", y="gop", hue="isFalse", ax=axes[axes_id,0])
...
if __name__ == "__main__":
    ...
    with open(sys.argv[1], "r") as ifile:
        for line in ifile:
            ...
            data_vec.append((fields[0], context_len, token_to, isFalse, gop))
            if hasOCC:
                occ = round(float(fields[4]),5)
    ...
    if hasOCC:
        ...
        full_context_stastics(df)
        print("after normalizaition:")
        full_context_stastics(df_occ)
        plot_it(sys.argv[2], df_occ)
        plot_it(sys.argv[2]+"_unorm", df)
    else:
        df = pd.DataFrame(data_vec, columns=('uttid','context-len','token', "isFalse", "gop"))
        full_context_stastics(df)  
