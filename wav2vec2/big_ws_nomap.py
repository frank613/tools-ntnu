...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        for row in dataset:
            ...
            sim_denom, index1 = similarity_num.max(-1)
            sim_denom, index2 = sim_denom.max(-1)
            for p in p_set:
                weight_tensor = torch.Tensor(prior[p]) #shape = (320,320)
                sim_num = (weight_tensor * similarity_num).sum(dim=(1,2))
                ...
                for phoneme,start_idx,end_idx in segmented:
                    if phoneme == "SIL" or phoneme == "SPN":
                        continue
                    gops_map[phoneme][p].append(sim_diff[start_idx:end_idx].mean())
