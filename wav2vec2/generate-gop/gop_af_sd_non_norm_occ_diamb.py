    ...
    with torch.no_grad(), open(out_path+"_"+proc_id+".txt", "a") as f:
        for i,uid in enumerate(batch["id"]):
            ...
            for i,pid in enumerate(pids):
                ...
                f.write("%d %s %s %s %s %s\n"%(i, p_tokenizer._convert_id_to_token(int(pid)), gop.item(), occ.item(), occ_extra.item(), occ_num.item()))
            f.write("\n")
...
if __name__ == "__main__":
    ...
    ds.map(single_process, fn_kwargs={"p_tokenizer":p_tokenizer, "processor":processor, "model_path":model_path, "out_path":sys.argv[6]},  batched=True, batch_size=50, num_proc=100) 
    print("done")
