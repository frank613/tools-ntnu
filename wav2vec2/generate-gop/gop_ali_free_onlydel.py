...
def ctc_loss_denom(params, seq, pos, blank=0):
    ...
    ## constraint mask for disabling insertion and substitution
    mask_ins = torch.zeros((P,P))
    ...
    if pos == 0:
        ...
        alphas[1,0] = torch.zeros_like(alphas[1,0])
    else:
        alphas[0,0,0] = params[blank,0]
        ...
        for s in range(start,L):
            ...
            if s%2 == 0:
                if s==0:
                    ...
                    if l == pos: ## the first blank after deleted phoneme
                        sum = 0
                        if t == 1: 
                ...
                alphas[s,t,0] = (alphas[s,t-1,0] +  sum ) * params[seq[l],t] + skip_empty + skip_token
                ...
                if s == 1: #the blank pathes from the first term is already removed for t=0 at initial step, so we don't do it again
                    empty_prob = torch.zeros_like(alphas[s,t,:])
                    ...
                    skip_prob = torch.zeros_like(alphas[s,t,:])
                    empty_prob = torch.zeros_like(alphas[s,t,:])

                    alphas[s,t,:] = (alphas[s,t-1,:].view(1,-1) * params[:,t].view(-1,1) * mask_ins).sum(-1) + skip_prob + empty_prob

          
    if pos == seqLen-1: # last label is arbitrary, inlcludes empty as well so we don't need the last term alphas[L-1,T-1,0], but we need the skip path alphas[L-3, T-1, 0] + alphas[L-4, T-1, 0]
