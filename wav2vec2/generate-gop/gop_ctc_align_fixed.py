            ...
            for i,state in enumerate(full_path_int):
                ...
                if state != last_state:
                    if post_count != 0: ##previous state is not blank, token->blank or token1->token2
                        ## fix to compute log first then average
                        gop_list.append((p_tokenizer._convert_id_to_token(pids[l]), post_log_total/post_count))
                        post_count = 0
                        post_log_total = 0
                    #else: # blank->token
                if state%2 != 0:
                    post_count += 1
                    post_log_total += torch.log(post_mat[pids[l_new],i])
                last_state = state
            if post_count != 0:
                l = int((last_state - 1)/2)
                gop_list.append((p_tokenizer._convert_id_to_token(pids[l]), post_log_total/post_count))
