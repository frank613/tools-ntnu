        ...
        for s in range(start,L):
            ...
            if s%2 == 0:
                if s==0:
                    ...
                    pointers[s,t] = s if winner  == alphas[s,t-1] else s-1
            else:
...
if __name__ == "__main__":
    ...
    print(sys.argv)
    if len(sys.argv) != 5:
        sys.exit("this script takes 4 arguments <w2v2-model-dir> <local-data-csv-folder> <w2v2-preprocessr-dir> <out-file>.\n \
            , it analyzes the GOP using the ctc's alignment, the csv path must be a folder containing audios files and the metadata.csv") 
