...
def writes(gops_list, outFile):
    ...
    with open(outFile, 'w') as fw:
        for key, gop_list in gops_list:
            fw.write(key+'\n')
            for cnt, (p,feats) in enumerate(gop_list):
                feat_s = ",".join([ str(torch.round(feat,decimals=3).numpy()) for feat in feats])
                fw.write("%d %s %s\n"%(cnt, p, feat_s))
            fw.write("\n")
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        for row in ds:
            ...
            pids = labels.tolist()
            gop_list = []
            num_non_blank = logits.shape[1] - 1
            for i,pid in enumerate(pids):
                gop_feats = [log_like_total]
                denom_total = 0
                for sub_pid in range(1, num_non_blank+1):
                ...
                gop_list.append((p_tokenizer._convert_id_to_token(pid), gop_feats))
