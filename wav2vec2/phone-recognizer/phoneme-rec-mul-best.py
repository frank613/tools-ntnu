...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        for row in dataset:
            ...
            results = model.wav2vec2(input_values)
            cnn_last_hidden = model.dropout_features(results[1]) 
            logits = model.quantizer.weight_proj(cnn_last_hidden)
            #softmax must be applied on each code group, shape=(N_frames, 2, 320)
            soft = logits.view(-1,num_groups,num_vars).softmax(dim=-1) 
            #expand col-wise
            soft1 = soft[:,0,:].unsqueeze(-1).expand(soft.shape[0], 320, 320)
            #expand row-wise
            soft2 = soft[:,1,:].unsqueeze(-2).expand(soft.shape[0], 320, 320)
            soft = (soft1 * soft2).unsqueeze(-1) #shape = (N_frames, 320, 320, 1)
            prob_seq = soft * prior_phoneme #shape = (N_frames, 320, 320, N_phonemes)
            prob_seq = prob_seq.view(prob_seq.shape[0], -1, prob_seq.shape[3]) #shape=(N_frames, 102400, N_phonemes)
            prob_seq = prob_seq.max(dim=1)[0] #shape=(N_frames,N_phonemes)
            id_seq = prob_seq.max(-1)[1]
