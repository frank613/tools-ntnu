...
def compute_gop_af_batch(post_mat, ll_self, batch_label, alphas, betas):
    ...
    s1 = 2*L-1 -1 - 1
    for t1 in range(T-1):
        index_list = torch.stack((torch.ones(P)*(t1+1), p_l),0).tolist()
        denom_sum[-P:] += betas[:P,s1,t1]*(1-post_mat[index_list]) 
        
    #for the rest [alpha_t1(s1) * (1-y(t1+1,P1)] * [(1 - y(t2-1,P2))*beta_t2(s2)]
    #t2 >= t1 + 3
    #p_l,p_m,p_r = list(zip(pids_shiftdown, pids, pids_shiftup))[order]  
    ...
    for t1 in range(T-3): 
        for t2 in range(t1+3,T):
            ...
            index_list_l = torch.stack((torch.ones(len(phoneme_number_seq))*(t1+1), p_l),0).tolist()
            alpha_idx = torch.stack((torch.arange(P,L*P-P), s1_batch, torch.ones(len(s1_batch))*t1),0).tolist()
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        for row in ds:
            ...
            batch_label = labels.repeat(label_length,len(pid_set),1)  #L*P*L
