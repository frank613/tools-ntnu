...
if __name__ == "__main__":
    ...
    if sys.argv[1] != "test" and sys.argv[1] != "validation":
        sys.exit("The first argument must be validation or test") 
    ...
    with torch.no_grad():
        ...
        for row in dataset:
            ...
            idx = logits.view(-1,num_vars).argmax(dim=-1)
            idx = idx.view(-1, num_groups).permute(1,0) #shape = (2, N_frames,)
            prior_seq = prior_phoneme[idx[0], idx[1]] #shape = (N_frames, N_phonemes)
        ...
        print("Total-non-zero: {0}, Error: {1}, The PER of {2} ({3} utterances) is {4}".format(total_frames_nonzero, total_error_nonzero, sys.argv[1], sys.argv[2], torch.round(total_error/total_frames, decimals=3)))
