...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        for row in dataset:
            ...
            map_denom = torch.cosine_similarity(projected_vector, closest_vector) #shape= (seq_len)
            ...
            best_denom, index1 = similarity_num.max(-1)
            best_denom, index2 = best_denom.max(-1)
            denom_diff = (map_denom - best_denom).round(decimals=3)
            for th in threshold:
                pdb.set_trace()
                res_list = []
                for phoneme,start_idx,end_idx in segmented:
                    if phoneme == "SIL" or phoneme == "SPN":
                        continue
                    weight_tensor = torch.Tensor(prior[phoneme]) #shape = (320,320)
                    weight_tensor = torch.where(weight_tensor >= th, 1, 0)
                    best, idx1 = sim_num = (weight_tensor * similarity_num[start_idx:end_idx]).max(-1)
                    best, idx2 = best.max(-1)

                    #step 5 gop calculation
                    sim_diff = best.mean() - best_denom[start_idx:end_idx] #shape= (seq_len)
                    res_list.append((phoneme, sim_diff.mean().round(decimals=3)))
