...
def ctc_ent_loss_log(pred, pred_len, token, token_len, blank=0):
    ...
    token_with_blank = T.cat(
        (T.zeros(batch, U, 1).type(longX), token[:, :, None]), dim=2).view(batch, -1)    # (batch, 2U)
...
def ctc_entropy_cost(out, targets, sizes, target_sizes, use_softmax=True, use_log=True, sumed=True, blank=0):
