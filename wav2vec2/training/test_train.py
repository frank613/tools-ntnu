...
def prepare_dataset(batch):
    ...
    ## is_split_into_words =True to disamibuate batched input or tokenzed single input.
    batch["labels"] = processor(text = token_batch, is_split_into_words= True).input_ids
    return batch
## batched only for tranformers>4.25.0 where features without padding is processable
def prepare_dataset_batch(batch):
    ...
    #id_batch =processor.tokenizer.convert_tokens_to_ids(token_batch)
    batch["labels"] = processor(text = token_batch).input_ids
    return batch
...
if __name__ == "__main__":
    ...
    #train_ds = load_dataset_local_from_dict(csv_path_dev, "dev", tran_map_dev, uttid_list_dev)
    ...
    vocabs_train = train_ds.map(extract_all_phonemes, batched=True, batch_size=-1, keep_in_memory=True, remove_columns=train_ds.column_names)
    ...
    cache_full_path_train = os.path.join(ds_cache_path, "train-trans")
    cache_full_path_dev = os.path.join(ds_cache_path, "dev-trans")
    if not os.path.exists(cache_full_path_train):
        #for transformers 4.25.0, we can't process features with arbitrary length in a batch
        #train_ds = train_ds.map(prepare_dataset_batch, batched=True, batch_size=100, num_proc=1)
        train_ds = train_ds.map(prepare_dataset, num_proc=10)
        train_ds.save_to_disk(cache_full_path_train)
    else:
        train_ds = datasets.Dataset.load_from_disk(cache_full_path_train)
    if not os.path.exists(cache_full_path_dev): 
        
        #dev_ds = dev_ds.map(prepare_dataset_batch, batched=True, batch_size=100,num_proc=1)
        dev_ds = dev_ds.map(prepare_dataset, num_proc=10)
        dev_ds.save_to_disk(cache_full_path_dev)
    else:
        dev_ds = datasets.Dataset.load_from_disk(cache_full_path_dev)
