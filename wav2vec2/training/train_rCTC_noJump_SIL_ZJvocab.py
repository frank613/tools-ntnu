...
from my_w2v2_package.custom_CTC_module import Wav2Vec2ForNOJCTC
...
if __name__ == "__main__":
    ...
    if os.path.isfile(model_vocab_path):
        jsonf = open(model_vocab_path)
        vocab_dict = json.load(jsonf)
        if "|" in vocab_dict.keys():
            vocab_dict.pop("|")
    ...
    ##load
    #audio_test = load_from_disk("./models/zijian-datasets/lbi-ctc-test")
    #step 4, fine-tune the model, the datacollator does the padding.
    model = Wav2Vec2ForNOJCTC.from_pretrained(
        model_path, 
